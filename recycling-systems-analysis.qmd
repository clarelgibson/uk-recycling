...
We now need to decide how many clusters *k* to divide the data into. We will use a dendrogram and a silhouette plot to aid this decision but we must also keep in mind that with `r nrow(recycling)` authorities and three unevenly sized collection systems, fine-grained clusters risk leaving peer groups with zero or one authority of some system. Stratification only controls confounding if every stratum still holds a spread of systems to compare. So *k* is bounded from above by hese factors, and not just by dendrogram or silhouette results.
